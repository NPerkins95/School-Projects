###############################################################################
# Title: Assign02P3			Author: Nathan Perkins
# Class: CS 2318-002, Fall 2016		Submitted: 10/31/2016
###############################################################################
...
#                     hopPtr1 = a1;		
					la $t5, a1	
#                     endPtr1 = a1 + used1;		
					sll $v1, $t1, 2			# $v1 now has 4 * used1
					add $a1, $t5, $v1 	
#                     goto FTest1;		
					j FTest1	
begF1:#//             {		
...
#                        if ( (target & 1) != 1) goto else3;		
					li $v1, 1	
					andi $t0, $t4, 1		# $t0 now has target & 1
					bne $t0, $v1, else3	
begI3:#//                {		
#                           hopPtr3 = a3 + used3 - 1;		
#                           endPtr3 = a3;		
					la $a3, a3			# $a3 has a3
					addi $v1, $t3, -1		# $v1 now has used3 - 1
					sll $v1, $v1, 2			# $v1 now has 4 * (used3 - 1)
					add $t7, $a3, $v1	
...
#                              if (*hopPtr3 <= target) goto else4;		
					lw $v1, 0($t7)			# $v1 now has *hopPtr3
					ble $v1, $t4, else4	
begI4:#//                      {		
#                                 *(hopPtr3 + 1) = *hopPtr3;		
					sw $v1, 4($t7)			# $v1 now has new *hopPtr3
#                                 --hopPtr3;		
					addi $t7, $t7, -4	
#                              goto endI4;		
					j endI4	
...
#                           hopPtr2 = a2;		
					la $t6, a2	
#                           endPtr2 = a2 + used2;		
					sll $v1, $t2, 2			# $v1 now has 4 * used2
					add $a2, $t6, $v1	
...
begW5:#//                         {		
#                                    *hopPtr21 = *(hopPtr21 - 1);		
					lw $v1, -4($t8)			# $v1 now has *(hopPtr21 - 1)
					sw $v1, 0($t8)	
#                                    --hopPtr21;		
					addi $t8, $t8, -4	
WTest5:#//                        }		
...
#                        hopPtr3 = a3;		
					la $t7, a3	
#                        endPtr3 = a3 + used3;		
					sll $v1, $t3, 2			# $v1 now has 4 * used3
					add $a3, $t7, $v1 	
#//                      do		
begDW3:#//               {		
#                           cout << *hopPtr3 << ' ' << ' ';		
...
#                        if (*hopPtr2 >= *hopPtr3) goto else9;		
					lw $v1, 0($t6)			# $v1 now has *hopPtr2
					lw $t0, 0($t7)			# $t0 now has *hopPtr3
					bge $v1, $t0, else9	
begI9:#//                {		
#                           *hopPtr1 = *hopPtr2;		
					sw $v1, 0($t5)			# $v1 now has new *hopPtr2
#                           ++hopPtr2;		
					addi $t6, $t6, 4	
#                        goto endI9;		
					j endI9	
...
#//                      {		
#                           *hopPtr1 = *hopPtr3;		
					sw $t0, 0($t5)			# $t0 now has new *hopPtr3
#                           ++hopPtr3;		
					addi $t7, $t7, 4	
endI9:#//                }		
#                        ++hopPtr1;		
...
#                        *hopPtr1 = *hopPtr2;		
					lw $v1, 0($t6)			# $v1 now has *hopPtr2
					sw $v1, 0($t5)			# *hopPtr1 now contains value for *hopPtr2
#                        ++hopPtr2;		
					addi $t6, $t6, 4	
#                        ++hopPtr1;		
					addi $t5, $t5, 4	
...
#                        *hopPtr1 = *hopPtr3;		
					lw $v1, 0($t7)			# $v1 now has *hopPtr3
					sw $v1, 0($t5)			# *hopPtr1 now contains value for *hopPtr3
#                        ++hopPtr3;		
					addi $t7, $t7, 4	
#                        ++hopPtr1;		
					addi $t5, $t5, 4	
